...
resource "time_sleep" "wait_for_apigw_integration_eu" {
  depends_on      = [aws_api_gateway_integration.userplatform_cpp_api_integration_eu]
  create_duration = "30s" # 20–45s is common; adjust if needed
}
...
resource "aws_api_gateway_deployment" "userplatform_cpp_api_deployment_eu" {
  ...
  depends_on = [
    time_sleep.wait_for_apigw_integration_eu,
    aws_api_gateway_integration.userplatform_cpp_api_integration_eu,
    aws_api_gateway_method_response.userplatform_cpp_apigateway_s3_method_response_eu,
