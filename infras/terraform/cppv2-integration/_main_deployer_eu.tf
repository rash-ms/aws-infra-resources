...
resource "aws_api_gateway_deployment" "userplatform_cpp_api_deployment_eu" {
  ...
  # triggers = {
  #   redeploy = sha1(jsonencode(aws_api_gateway_integration.userplatform_cpp_api_integration_eu.request_templates))
  # }

  triggers = {
    redeploy = sha1(jsonencode({
      method_id               = aws_api_gateway_method.userplatform_cpp_api_method_eu.id
  ...
  lifecycle {
    ...
    replace_triggered_by = [
      aws_api_gateway_integration.userplatform_cpp_api_integration_eu.id,
      aws_api_gateway_method.userplatform_cpp_api_method_eu.id
    ]
  }
}
